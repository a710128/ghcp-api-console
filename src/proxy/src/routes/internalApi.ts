...
internalApiRouter.put('/accounts/:identity/gh-token', async (req, res) => {
  ...
  const account = await saveGithubToken(req.params.identity, ghToken, typeof ghLogin === 'string' ? ghLogin : undefined);
...
internalApiRouter.post('/accounts/:identity/mark-gh-token-failed', async (req, res) => {
  ...
  await markGithubTokenStatus(req.params.identity, 'failed');
